...
const Home = () => {
  ...
  return (
    <div className={`space-y-12 pb-20 ${simpleMode ? 'simple-mode-active' : ''}`}>
      {/* Welcome Header */}
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 animate-in fade-in slide-in-from-top duration-700">
        ...
        <div className="flex items-center gap-3 bg-card border border-border p-2 rounded-2xl shadow-sm">
          <div className="px-4 py-2 bg-primary/10 text-primary rounded-xl text-xs font-black uppercase tracking-widest" aria-label={`Current status: ${currentStage === 0 ? 'Getting Started' : 'Stage ' + (currentStage + 1) + ' Active'}`}>
            {currentStage === 0 ? 'Getting Started' : `Stage ${currentStage + 1} Active`}
          </div>
          <button 
            onClick={() => navigate('/settings')}
            className="w-10 h-10 flex items-center justify-center text-text-secondary hover:text-primary transition-colors focus-visible:ring-2 focus-visible:ring-primary rounded-xl"
            ...
            <div className="flex-1 space-y-8 text-center lg:text-left">
              ...
              <div className="flex flex-wrap justify-center lg:justify-start gap-4 pt-4">
                <Button 
                  ...
                  ariaLabel="Resume Guided Journey"
                >
                  Resume Journey
                </Button>
                <Button 
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8" aria-label="Election Statistics">
        {(simpleMode ? stats.slice(0, 1) : stats).map((stat) => (
          <Card key={stat.label} padding="p-8" className="flex items-center gap-6 group" hover={false}>
            ...
            <div>
              ...
              <div className="flex items-end gap-3">
                ...
                {!simpleMode && (
                  <span className={`text-[10px] font-black px-2 py-1 rounded ${stat.change.includes('+') ? 'bg-accent/10 text-accent' : 'bg-primary/10 text-primary'}`} aria-label={`Change: ${stat.change}`}>
                    {stat.change}
                  </span>
                )}
              </div>
      ...
      {/* Features Grid */}
      <section className="space-y-10" aria-label="Learning Modules">
        <div className="flex items-end justify-between px-2">
          <div>
            <h2 className="text-3xl font-black text-text-primary tracking-tight">Learning Modules</h2>
            <p className="text-text-secondary mt-1 text-lg font-medium opacity-60">Deep dive into specific election domains.</p>
          </div>
          <button 
            className="text-xs font-black text-primary uppercase tracking-widest hover:underline underline-offset-8 decoration-2"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature) => (
            <Card
              ...
              ariaLabel={`Go to ${feature.title}`}
            >
              <div className={`absolute top-0 right-0 w-32 h-32 ${feature.color} opacity-0 group-hover:opacity-10 rounded-bl-[5rem] transition-all duration-500`}></div>
              ...
              <div className="mt-4 flex items-center gap-2 text-[10px] font-black text-primary tracking-[0.2em] uppercase">
                Explore <span className="material-symbols-outlined text-sm group-hover:translate-x-2 transition-transform">arrow_right_alt</span>
              </div>
            </Card>
          ))}
        </div>
